	...
	function SimpleWebAudioStreamPlayer (options) {
		...
		this.bufferToArray = function(buffer) { // incoming data is an ArrayBuffer
			var u16arrayData = new Uint16Array(buffer);
			var outputArrayData = new Float32Array(u16arrayData.length);
			for (var i = 0; i < u16arrayData.length; i++) {
				// If the high bit is on, then it is a negative number, and actually counts backwards.
				outputArrayData[i] = (u16arrayData[i] >= 0x8000) ? -(0x10000 - u16arrayData[i]) / 0x8000 : u16arrayData[i] / 0x7FFF;
			}
		...
		*/
		...
		this.bufferToArray = function(buffer) { // incoming data is an ArrayBuffer
			var u16arrayData = new Uint16Array(buffer);
			var outputArrayData1 = new Float32Array(u16arrayData.length / 2);
			var outputArrayData2 = new Float32Array(u16arrayData.length / 2);
			var x = 0; var y = 0;
			for (var i = 0; i < u16arrayData.length; i++) {
				if (i % 2 == 0) outputArrayData1[x++] = (u16arrayData[i] >= 0x8000) 
														? -(0x10000 - u16arrayData[i]) / 0x8000 
														: u16arrayData[i] / 0x7FFF;
				else outputArrayData2[y++] = (u16arrayData[i] >= 0x8000) 
														? -(0x10000 - u16arrayData[i]) / 0x8000 
														: u16arrayData[i] / 0x7FFF;
			}
			return [outputArrayData1, outputArrayData2]; // return the Float32Array
		};
		...
		this.bufferToArray = function(buffer) { // incoming data is an ArrayBuffer
			var u8arrayData = new Uint8Array(buffer);
			var outputArrayData1 = new Float32Array(u8arrayData.length / 2);
			var outputArrayData2 = new Float32Array(u8arrayData.length / 2);
			var x = 0; var y = 0;
			for (var i = 0; i < u8arrayData.length; i++) {
				if (i % 2 == 0) outputArrayData1[x++] = (u8arrayData[i] - 128) / 128.0; // convert audio to float
				else outputArrayData2[y++] = (u8arrayData[i] - 128) / 128.0; // convert audio to float
			}
			return [outputArrayData1, outputArrayData2]; // return the Float32Array
		};
		*/
		...
		this.prepareAudioBuffer = function(buffer, channelCount) {
			...
			try {
				...
				for (var i=0; i<this.inputChannelCount; i++) {
					...
					var intputBuffer = channelCount <= 0 ? buffer.getChannelData(i) : (buffer.length == 2 ? buffer[i] : buffer);
